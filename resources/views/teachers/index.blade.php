    ...
    <div class="bg-white rounded-[2.5rem] shadow-[0_10px_40px_rgba(0,0,0,0.03)] border border-gray-50 overflow-hidden">
        <div class="px-10 py-4 border-b border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div class="flex items-center gap-3 w-full md:w-auto">
                ...
                <a href="{{ route('teachers.create') }}" class="bg-gray-100 hover:bg-gray-200 text-[#345344] text-sm font-bold px-6 h-[42px] rounded-xl transition-all border border-gray-100 flex items-center gap-2 whitespace-nowrap">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50">
                        <th class="px-10 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">Nama Guru</th>
                        ...
                        <tr>
                            <td class="px-10 py-3">
                                <div class="flex items-center gap-4">
                                    <div>
                                        <p class="text-sm font-bold text-gray-800 leading-none">{{ $teacher->name }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-10 py-3">
                                <span class="text-sm font-medium text-gray-500">{{ $teacher->username }}</span>
                            </td>
                            ...
                            <td class="px-10 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <form id="delete-teacher-{{ $teacher->id }}" action="{{ route('teachers.destroy', $teacher) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="confirmDelete('delete-teacher-{{ $teacher->id }}', '{{ $teacher->name }}')" class="p-2.5 rounded-xl bg-gray-50 text-gray-400 hover:bg-red-50 hover:text-red-500 transition-all" title="Hapus">
